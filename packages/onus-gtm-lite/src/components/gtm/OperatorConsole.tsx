...
export function OperatorConsole({
  ...
  const events = useMemo(() => {
    ...
    if (newsQuery) {
      out.push({ t: stamp(12), k: "news.query", v: newsQuery.slice(0, 80) });
    }
    if (newsFetched !== undefined) {
      out.push({ t: stamp(10), k: "news.fetched", v: `n=${newsFetched}` });
    }
    if (newsSelected !== undefined) {
      out.push({ t: stamp(8), k: "news.selected", v: `n=${newsSelected}` });
    }
